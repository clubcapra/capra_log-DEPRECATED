...
public class TextFileLoader
{
	...
	{
		_file = inFile;
		/*if(_file.getName().contains("-gps") && preCheckFile(_file)){
		...
		if (preCheckFile(_file))
		{
			...
			if(_file.getName().contains("-gps")){

				//First panel
				_delimiterRadios[2].doClick();
				_nextButton.doClick();	
				
				//Second panel
				Field[] tabField = _fieldTableModel.getFieldArray();
				_fieldTableModel.setValueAt(Field.LONGITUDE, 0, 1);
				_fieldTableModel.setValueAt(Field.LATITUDE, 1, 1);
				tabField[2] = Field.WAYPT_NAME;
				_nextButton.doClick();
	...
	public DelimiterInfo getSelectedDelimiterInfo()
	{
		...
		if(_delimiterRadios == null) return _delimiterInfos[2];
		else{
			for (int i=0; i<4; i++)
