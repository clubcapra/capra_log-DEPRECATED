...
public class FileSaver
{
		...
		int inCoordFormat, Unit inAltitudeUnit, Timestamp.Format inTimestampFormat)
	{
		...
		{
			inBuffer.append("x=\"" + inPoint.getLatitude().output(inCoordFormat) + "\"");
		}
		else if (inField == Field.LONGITUDE)
		{
			inBuffer.append("y=\"" + inPoint.getLongitude().output(inCoordFormat) + "\"");
		}
	...
	private static boolean isFilenameOk(File inFile)
	{
		...
		return (filename.length() <4 || (!filename.endsWith(".gpx") && !filename.endsWith(".xml")
